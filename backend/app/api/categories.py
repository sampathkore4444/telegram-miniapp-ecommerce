...
async def list_categories(db: DbDep, store: ActiveStore, include_inactive: bool = False):
...
async def get_category(slug: str, db: DbDep, store: ActiveStore):
    from sqlalchemy import select

    result = await db.execute(
        select(Category).where(Category.slug == slug, Category.store_id == store.id)
    )
